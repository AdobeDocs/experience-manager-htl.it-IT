...
**HTL supporta le librerie di tag JSP?** - No, ma come mostrato nella sezione  [Caricamento ](getting-started.md#loading-client-libraries) delle librerie client, il  [modello e le istruzioni di ](block-statements.md#template-call) chiamata offrono un pattern simile.

**È possibile estendere le funzioni HTL su un progetto AEM?** - No, non possono. HTL dispone di potenti meccanismi di estensione per il riutilizzo della logica - le [Use-API](getting-started.md#use-api-for-accessing-logic) - e della marcatura (le istruzioni [template &amp; call](block-statements.md#template-call)), che possono essere utilizzate per modulare il codice dei progetti.
...
Di seguito è riportato un primo esempio, che potrebbe essere contenuto come in un file **`template.html`**:
...
restituirà un elemento simile al seguente HTML, ma solo se sono definite entrambe le proprietà **`jcr:title`** e **`jcr:description`** e se nessuna delle due è vuota:
...
Il motivo è che il contenuto di questi contesti è testo e non HTML, e gli elementi HTML contenuti sarebbero considerati come dati di carattere semplici. Pertanto, senza elementi HTML reali, non è possibile eseguire anche gli attributi **`data-sly`**.

Questo può sembrare una grande restrizione, ma è una restrizione desiderata, perché il linguaggio HTML Template Language non dovrebbe essere abusato per generare un output che non è HTML. La sezione [Use-API for Accessing Logic](getting-started.md#use-api-for-accessing-logic) di seguito illustra come è possibile chiamare logica aggiuntiva dal modello, che può essere utilizzata se è necessaria per preparare output complessi per questi contesti. Ad esempio, un modo semplice per inviare i dati dal back-end a uno script front-end consiste nel fare in modo che la logica del componente generi una stringa JSON, che può quindi essere inserita in un attributo di dati con una semplice espressione HTL.
...
Come spiegato nella sezione [Automatic Context-Aware Escaping](getting-started.md#automatic-context-aware-escaping) di seguito, uno degli obiettivi di HTL è quello di ridurre i rischi legati all&#39;introduzione di vulnerabilità di scripting tra siti (XSS) applicando automaticamente la escape in base al contesto a tutte le espressioni. HTL è in grado di rilevare automaticamente il contesto delle espressioni inserite all&#39;interno di tag HTML, ma non analizza la sintassi di JavaScript o CSS in linea e pertanto si affida allo sviluppatore per specificare esplicitamente quale contesto esatto debba essere applicato a tali espressioni.
...
Per ulteriori dettagli su come controllare la escape, fare riferimento alla sezione [Contesto di visualizzazione della lingua di espressione](expression-language.md#display-context).
...
Per un esempio, vedere la sezione [Utilizzo dei modelli lato client](getting-started.md#working-with-client-side-templates).
...
E il seguente file JavaScript eseguito sul lato server `logic.js` posizionato accanto ad esso:
...
* La sezione sull&#39;istruzione [`data-sly-use`](block-statements.md#use) spiega tutto ciò che è possibile fare con tale istruzione.
* La [pagina Use-API](use-api.md) fornisce alcune informazioni che aiutano a scegliere se scrivere la logica in Java o in JavaScript.
* Per informazioni dettagliate su come scrivere la logica, è necessario consultare le pagine [JavaScript Use-API](use-api-javascript.md) e [Java Use-API](use-api-java.md).

### Rimozione automatica in base al contesto {#automatic-context-aware-escaping}
...
Nella maggior parte dei linguaggi dei modelli, questo esempio potrebbe creare una vulnerabilità di scripting tra siti (XSS), perché anche quando tutte le variabili sono sottoposte automaticamente a escape HTML, l&#39;attributo `href` deve ancora essere caratterizzato da una specifica escape URL. Questa omissione è uno degli errori più comuni, perché può essere facilmente dimenticata, ed è difficile individuare in modo automatizzato.
...
Notate come i due attributi sono stati sottoposti a escape in modo diverso, perché HTL sa che gli attributi `href` e `src` devono essere preceduti da un carattere di escape per il contesto URI. Inoltre, se l&#39;URI iniziasse con **`javascript:`**, l&#39;attributo sarebbe stato rimosso completamente, a meno che il contesto non fosse stato esplicitamente modificato in altro modo.
...
Se il valore della proprietà `class` è vuoto, il linguaggio HTML Template Language rimuove automaticamente l&#39;intero attributo `class` dall&#39;output.
...
Per l&#39;impostazione degli attributi, l&#39;istruzione [`data-sly-attribute`](block-statements.md#attribute) potrebbe essere utile.
...
Nel secondo esempio precedente, nel caso in cui gli elementi HTML **`head`** e **`body`** siano inseriti in file diversi, il modello **`clientlib.html`** dovrebbe essere caricato in ciascun file che ne ha bisogno.

La sezione sulle istruzioni [template &amp; call](block-statements.md#template-call) fornisce ulteriori dettagli sul funzionamento di tali modelli.
...
Un caso speciale, in cui la tecnica spiegata nella sezione [Limitazioni di sollevamento dei contesti speciali](getting-started.md#lifting-limitations-of-special-contexts) può essere utilizzata legittimamente, è quello di scrivere modelli lato client (come ad esempio Handlebars) che si trovano all&#39;interno di elementi **script**. Il motivo per cui questa tecnica può essere utilizzata in questo caso è perché l&#39;elemento **script** non contiene JavaScript come ipotizzato, ma ulteriori elementi HTML. Ecco un esempio di come potrebbe funzionare:
...
Come mostrato sopra, la marcatura che verrà inclusa nell&#39;elemento **`script`** può contenere istruzioni di blocco HTL e le espressioni non devono fornire contesti espliciti, perché il contenuto del modello Handlebars è stato isolato nel proprio file. Inoltre, questo esempio mostra come l&#39;HTL eseguito sul lato server (come nell&#39;elemento **`h2`**) possa essere combinato con un linguaggio modello eseguito sul lato client, come Handlebars (mostrato nell&#39;elemento **`h3`**).

Una tecnica più moderna, tuttavia, sarebbe utilizzare l&#39;elemento HTML **`template`**, in quanto il vantaggio sarebbe che non è necessario isolare il contenuto dei modelli in file separati.
